...
local AnimalPed = {
    ...
    joaat("whitecat"), 
    joaat("lightingcat"), 
    joaat("a_c_fantasydeer"), 
    joaat("a_c_lionmount"), 
    joaat("a_c_skelhorse"), 
}

local function IsAnimalPed()
	local model = GetEntityModel(PlayerPedId(-1))
    for _, animalModel in ipairs(AnimalPed) do
        if model == animalModel then
            return true
        end
    end
    return false
...
local function registerCommands()
	...
	local primary = lib.addKeybind({
		...
		onPressed = function()
			print('open_inventory event 1 ', invOpen)
			if invOpen then
				return client.closeInventory()
			end
			print('open_inventory event 2 ',cache.vehicle)

			if cache.vehicle then
				return openGlovebox(cache.vehicle)
			end

			print('open_inventory event 3 ')
			local closest = lib.points.getClosestPoint()

			print('open_inventory event 4 ',closest,closest.currentDistance,closest.instance,currentInstance)
...
local function createDrop(dropId, data)
	...
	print('Drop Event',data.coords,dropId,data.instance,data.model)

	print('point.model Event',point.model , client.dropprops)
	...
	print('point.model Event',point.distance,point.onEnter,point.onExit,point.nearby)
			...
			if not canOpenInventory() then
				...
				if currentInventory and not currentInventory.ignoreSecurityChecks then
					...
					if currentInventory.type == 'otherplayer' then
						...
						if not id or #(playerCoords - pedCoords) > 3.0 then -- or not (client.hasGroup(shared.police) or canOpenTarget(ped)) then
							client.closeInventory()
					...
					elseif currentInventory.coords and #(playerCoords - currentInventory.coords) > (currentInventory.distance or 3.0) then -- or canOpenTarget(playerPed)) then
						client.closeInventory()
						lib.notify({ id = 'inventory_lost_access', type = 'error', description = locale('inventory_lost_access') })
					end
				end
